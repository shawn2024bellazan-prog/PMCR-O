...
namespace Microsoft.Extensions.Hosting
{
    ...
    public static class Extensions
    {
        ...
        public static TBuilder AddServiceDefaults<TBuilder>(this TBuilder builder) where TBuilder : IHostApplicationBuilder
        {
            ...
            {
                // Extend the underlying socket timeout for LLM inference
                http.ConfigureHttpClient(client => client.Timeout = TimeSpan.FromMinutes(10));

                // Turn on resilience by default, but extend timeouts for local LLM inference
                http.AddStandardResilienceHandler().Configure(options =>
                {
                    options.AttemptTimeout.Timeout = TimeSpan.FromMinutes(10);
                    options.TotalRequestTimeout.Timeout = TimeSpan.FromMinutes(10);

                    // The Circuit Breaker sampling duration must be at least double the AttemptTimeout
